...
const HistoryPage: React.FC = () => {
  ...
  const [individualUserData, setIndividualUserData] = useState<UserData | null>(null);
  ...
  async function getCoordinatorHistory() {
    setLoading(true);
    try {
      const response = await axios.get("/api/Principal?action=getCoordinatorHistory");
    ...
    <div className="min-h-screen w-full bg-[#ffffff] flex flex-col gap-5 justify-start items-center">
      ...
      <div className="w-11/12 mt-5 mb-10 flex-col flex gap-2">
        <div className="flex w-full justify-between">
          {/* Heading */}
          <span className="text-4xl text-black font-semibold">All Applications</span>
          {/* Filter */}
          <div></div>
        </div>
        <div className="w-full mt-5 flex-col flex gap-5">
          {studentData.length > 0 ? (
            studentData.map((user) => (
              ...
              >
                <Application userInfo={user} isStudent={false} />
              </div>
            ))
          ) : (
